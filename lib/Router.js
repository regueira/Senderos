...
class Router {
    ...
    _verb( pathName, verb, cb ) {
        ...
        pathName = this.root !== '/' ? this.root + this._clearSlashes( pathName ) : this._clearSlashes( pathName );
    ...
    navigate( pathName, verb = 'get', data = {}, options = {} ) {
        ...
        let reg = new RegExp( '^' + this.root );
        pathName = this._clearSlashes( pathName );

        if ( this.root !== '/' ) {

            if ( pathName.match( reg ) ) {
                let reg1 = new RegExp( this.root + '$' );
                if ( pathName.match( reg1 ) ) {
                    pathName += '/';
                }
            } else {
